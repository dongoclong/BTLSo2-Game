...
public class Bomber extends Character {
    ...
    private int countene = 0  ;
    private  final int sizen = 0;
    private int power;
 ...
 // cout --
    ...
    public void handleCollisions(){ // xử lý va chạm cho bomber và các thực thể
        ...
        for (Entity stillObject : stillObjects) { // duyệt all thực thể
            ...
            if (r1.intersects(r2)) { // nếu bomber va chạm với các vật thể thì trả về true
                if ( stillObject instanceof Item) { // nêú chỉ số va chạm của bomber = thực thể và thực thể đó là item
                    ...
                    else if(stillObject instanceof DetonatorItem) { // tương tự bombitem
                        Sound.play("eat_item");
                        cout++;

                        stillObjects.remove(stillObject);
                    }
                    else if(stillObject instanceof RandomItem) { // tương tự bombitem
                        Sound.play("eat_item");
                        Random nRan = new Random();
                        int numR = nRan.nextInt(5)+1;
                        if(numR == 1) {
                            startFlame ++;
                            bomberman.setRadius(startFlame);
                            stillObjects.remove(stillObject);
                    ...
                    } else if(stillObject instanceof WallPassItem) { // tương tự bombitem
                        Sound.play("eat_item");
                    ...
                    else if(stillObject instanceof EnemiKillItem) { // tương tự bombitem
                        Sound.play("eat_item");
                        pass = true;
                ...
                } else if( stillObject instanceof Portal) { // chỉ số va chạm của bomber bằng thực thể và thực thể đó là portal
        ...
        for (Enemy enemy : enemies) {
            ...
            if (r1.intersects(r2) && pass == false) { // dkien bomber va chạm enemy

                enemy.setAlive(false);
                bomberman.setAlive(false);// cài bomber chết


                Sound.play("die"); // âm bomber chết
            ...
            } else if (r1.intersects(r2) && pass == true) { // dkien bomber va chạm enemy
                countene++;
                enemy.setAlive(false);
                System.out.println(countene);

                 if (countene > 90 ) { //giết dc 2 con se het bat tu
                     pass = false;
                 }
                //bomberman.setAlive(true);// cài bomber chết
